...
import { RefreshCw, Search, SlidersHorizontal, Ticket } from "lucide-react";
...
function badgeTone(badge: string) {
  ...
  if (badge === "Resident Deal") return "bg-[#e4f4ff] text-[#0b5c87]";
  ...
  const filteredDeals = useMemo(() => {
    ...
    return source.filter((deal) => {
      const matchesFilter =
        ...
        deal.badge === activeFilter;

      const searchable = `${deal.title} ${deal.city} ${deal.category} ${deal.description} ${deal.badge}`.toLowerCase();
      return matchesFilter && (!normalizedQuery || searchable.includes(normalizedQuery));
    });
  }, [activeFilter, query, shuffleSeed]);

  return (
    <section id="deals" className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-sm font-bold uppercase tracking-[0.18em] text-[#087f8c]">Today&apos;s Florida finds</p>
          <h2 className="mt-2 text-3xl font-black text-[#163235] sm:text-4xl">Browse live-style local deals</h2>
          <p className="mt-3 max-w-2xl text-base leading-7 text-[#52686b]">
            Filter by city, interest, price point, or resident-friendly savings across Florida&apos;s busiest travel markets.
          </p>
        </div>
        <button
      ...
      <div className="mt-7 rounded-[28px] border border-[#d8e6e3] bg-white p-3 shadow-xl shadow-[#087f8c]/8">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <div className="relative min-w-0 flex-1">
            ...
            <input
              aria-label="Search Florida deals"
              ...
              placeholder="Search deals, cities, events..."
              value={query}
            />
          </div>
      ...
      <div className="deal-scroll mt-7 grid max-h-[920px] gap-5 overflow-y-auto pr-1 sm:grid-cols-2 lg:grid-cols-3">
        {filteredDeals.map((deal) => (
          <article
            className="card-lift overflow-hidden rounded-[22px] border border-[#d8e6e3] bg-white shadow-lg shadow-[#087f8c]/8"
            key={deal.id}
          >
            <div className="relative h-44 overflow-hidden bg-[#dff6f8]">
              <img alt="" className="h-full w-full object-cover" loading="lazy" src={deal.image} />
            ...
            <div className="p-5">
              <div className="flex flex-wrap items-center gap-2 text-xs font-black uppercase tracking-[0.14em] text-[#087f8c]">
                ...
                <span>{categorySet.has(deal.category) ? deal.category : "Deal"}</span>
              </div>
